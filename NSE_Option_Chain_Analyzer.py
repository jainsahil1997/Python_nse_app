...
class Nse:
    ...
    def __init__(self, window: Tk) -> None:
        ...
        self.url_icon_png: str = "https://raw.githubusercontent.com/jainsahil1997/" \
                                 "Python-NSE-Option-Chain-Analyzer/master/nse_logo.png"
        self.url_icon_ico: str = "https://raw.githubusercontent.com/jainsahil1997/" \
    ...
    def links(self, link: str, event: Optional[Event] = None) -> None:
        ...
        if link == "developer":
            webbrowser.open_new("https://github.com/jainsahil1997/")
        elif link == "readme":
            webbrowser.open_new("https://github.com/jainsahil1997/Python-NSE-Option-Chain-Analyzer/blob/master/README.md/")
        elif link == "license":
            webbrowser.open_new("https://github.com/jainsahil1997/Python-NSE-Option-Chain-Analyzer/blob/master/LICENSE/")
        elif link == "releases":
            webbrowser.open_new("https://github.com/jainsahil1997/Python-NSE-Option-Chain-Analyzer/releases/")
        elif link == "sources":
            webbrowser.open_new("https://github.com/jainsahil1997/Python-NSE-Option-Chain-Analyzer/")
    ...
    def about(self, event: Optional[Event] = None) -> None:
        ...
        dev_val: Label = Label(self.info, text="Sahil Jain", fg="blue", cursor="hand2", relief=RIDGE)
    ...
    def get_dataframe(self) -> Optional[Tuple[pandas.DataFrame, str, float]]:
        ...
        spot_price=int(points)
        self.atm=spot_price if spot_price % 100 == 0 else spot_price + 100 - spot_price % 100
        if points == 0:
    ...
    def main(self) -> None:
        ...
        for t in reversed(range(0,10)):
            stp=self.atm-t*multiplication_factor
            stp_list.append(stp)
        t=0
        for t in range(1,10):
            stp=self.atm+t*multiplication_factor
        ...
        st_oi_list: List[int] = []
        for i in range(len(entire_oc)):
            int_call_oi: int = int(entire_oc.iloc[i, [10]][0])
            st_oi_list.append(int_call_oi)
        ciodata=pandas.DataFrame()
        t=0
        call_coi_list: List[int] = []
        call_ccoi_list: List[int] = []
        put_coi_list: List[int] = []
        put_ccoi_list: List[int] = []
        oi: numpy.float64
        for t in stp_list:
            #print(t)
            call_oi_index=st_oi_list.index(t)
            oi: numpy.float64 = entire_oc.iloc[call_oi_index,[0]][0]
            call_coi_list.append(oi)
            coi: int = entire_oc.iloc[call_oi_index,[1]][0]
            call_ccoi_list.append(coi)
            oi: numpy.float64 = entire_oc.iloc[call_oi_index,[20]][0]
            put_coi_list.append(oi)
            coi: int = entire_oc.iloc[call_oi_index,[19]][0]
            put_ccoi_list.append(coi)
        #print (coi_list)
        #print(ccoi_list)
        ciodata['Call_OI']=call_coi_list
        ciodata['Call_COI']=call_ccoi_list
        ciodata['strikePrice']=stp_list
        ciodata['PUT_OI']=put_coi_list
        ciodata['PUT_COI']=put_ccoi_list
        ce_data_f: pandas.DataFrame = entire_oc.loc[entire_oc['Strike Price'] == 36000]
        print(ciodata)
        total_put_oi=ciodata['PUT_COI'].sum()
        total_call_oi=ciodata['Call_COI'].sum()
        print("PE - CE ")
        print(total_put_oi-total_call_oi)
        if ( total_put_oi - total_call_oi > 0):
            print("Bullish")
        else:
            print ("Bearish")
